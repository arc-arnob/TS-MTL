...
from .model_components import FedAVGModel, ProximalTerm, DPOptimizer, DPOptimizerWithProximal, SecureAggregation, PrivacyPreservingFedAVGModel, ScaffoldControlVariate, DPOptimizerWithScaffold, PrivacyPreservingScaffoldModel
...
class PrivacyPreservingScaffoldSystem:
    """Privacy-preserving federated learning system with SCAFFOLD and secure aggregation."""
    def __init__(self, client_models, client_optimizers=None, 
        ...
        model_structure = self.client_models[0].get_model_structure()
        
        # Server control variate
        self.server_control_variate = ScaffoldControlVariate(model_structure).to(self.device)
        
        # Client control variates
        self.client_control_variates = [
            ScaffoldControlVariate(model_structure).to(self.device) 
        ...
        # Wrap optimizers with DP and SCAFFOLD
        self.client_optimizers = [
            DPOptimizerWithScaffold(
                optimizer=optimizer,
                model=model,
                noise_scale=noise_scale, 
                max_grad_norm=clip_norm
            )
            for optimizer, model in zip(self.base_optimizers, self.client_models)
        ]
    ...
    def compute_control_variate_update_option_ii(self, client_idx, initial_params, final_params, 
                                                  local_epochs, local_lr):
        """
        Compute control variate update using Option II from SCAFFOLD paper.
        c_i^+ = c_i - c + (1/(K*η_l)) * (x - y_i)
        """
        update = {}
        server_cv = self.server_control_variate.get_dict()
        client_cv = self.client_control_variates[client_idx].get_dict()
        
        for name in initial_params:
            if name in server_cv and name in client_cv:
                param_diff = initial_params[name] - final_params[name]
                control_update = (client_cv[name] - server_cv[name] + 
                                param_diff / (local_epochs * local_lr))
                update[name] = control_update.clone().detach()
            else:
                # Fallback: estimate from parameter change
                param_diff = initial_params[name] - final_params[name]
                update[name] = param_diff / (local_epochs * local_lr)
        
        return update
    
    def train_round(self, client_data_loaders, local_epochs=1, client_weights=None):
        """
        Perform one round of private SCAFFOLD training.
        """
        # Set models to training mode
        for model in self.client_models:
            model.train()
        ...
        # Step 1: Local client training with SCAFFOLD
        client_updates = []
        ...
        for client_idx, data_loader in enumerate(client_data_loaders):
            ...
            # Store initial parameters with names for control variate computation
            initial_params = {name: param.clone().detach() 
                            for name, param in self.client_models[client_idx].named_parameters()}
            
            # Get current control variates
            server_cv = self.server_control_variate.get_dict()
            client_cv = self.client_control_variates[client_idx].get_dict()
            
            # Local training with SCAFFOLD
            for epoch in range(local_epochs):
                print(f"Client {client_idx}, Epoch: {epoch}")
                epoch_loss = 0.0
                batch_count = 0
                ...
                for batch in data_loader:
                    ...
                    # Step with SCAFFOLD optimizer (handles control variates, clipping and noise)
                    self.client_optimizers[client_idx].step(
                        server_control_variate=server_cv,
                        client_control_variate=client_cv
                    )
            ...
            # Compute model update
            final_params = {name: param.clone().detach() 
                          for name, param in self.client_models[client_idx].named_parameters()}
            
            client_update = {}
            for name in final_params.keys():
                client_update[name] = final_params[name] - initial_params[name]
            
            client_updates.append(client_update)
            
            # Compute control variate update using Option II
            control_update = self.compute_control_variate_update_option_ii(
                client_idx, initial_params, final_params, 
                local_epochs, self.base_optimizers[client_idx].param_groups[0]['lr']
            )
            client_control_updates.append(control_update)
        
        # Step 2: Secure aggregation of updates
        if self.enable_secure_agg:
            # Get model shape dictionary based on named parameters
            model_shape_dict = {name: param.shape for name, param in self.client_models[0].named_parameters()}
            
            # Generate masking vectors for model updates
            model_masks = self.secure_agg.generate_masking_vectors(model_shape_dict, self.num_clients)
            
            # Generate masking vectors for control variate updates  
            control_masks = self.secure_agg.generate_masking_vectors(model_shape_dict, self.num_clients)
            
            # Apply masks
            masked_model_updates = [
                self.secure_agg.apply_mask(update, mask)
                for update, mask in zip(client_updates, model_masks)
            ]
            
            masked_control_updates = [
                self.secure_agg.apply_mask(update, mask)
                for update, mask in zip(client_control_updates, control_masks)
            ]
            
            # Server aggregates masked updates
            aggregated_model_update = {}
            aggregated_control_update = {}
            
            for key in model_shape_dict:
                aggregated_model_update[key] = sum(
                    masked_model_updates[i][key] * client_weights[i] for i in range(self.num_clients)
                )
                aggregated_control_update[key] = sum(
                    masked_control_updates[i][key] / self.num_clients for i in range(self.num_clients)
                )
        else:
            # Standard weighted aggregation (non-secure)
            aggregated_model_update = {}
            ...
            for key in client_updates[0].keys():
                ...
                aggregated_control_update[key] = sum(
                    client_control_updates[i][key] / self.num_clients for i in range(self.num_clients)
                )
        
        # Step 3: Update global model and control variates
        for model in self.client_models:
            # Apply aggregated model updates to each parameter
            with torch.no_grad():
                for name, param in model.named_parameters():
                    if name in aggregated_model_update:
                        param.add_(aggregated_model_update[name])
        
        # Update server control variate
        current_server_cv = self.server_control_variate.get_dict()
        for key in current_server_cv:
            if key in aggregated_control_update:
                current_server_cv[key] = current_server_cv[key] + aggregated_control_update[key]
        self.server_control_variate.update_from_dict(current_server_cv)
        
        # Update client control variates
        for client_idx in range(self.num_clients):
            current_client_cv = self.client_control_variates[client_idx].get_dict()
            for key in current_client_cv:
                if key in client_control_updates[client_idx]:
                    current_client_cv[key] = current_client_cv[key] + client_control_updates[client_idx][key]
            self.client_control_variates[client_idx].update_from_dict(current_client_cv)
        ...
        for client_idx, data_loader in enumerate(client_data_loaders):
            ...
            for epoch in range(personalization_epochs):
                ...
                for batch in data_loader:
                    ...
                    # Step with personalized DP optimizer (no SCAFFOLD correction in personalization)
                    self.personalized_optimizers[client_idx].step()
...
def train_private_scaffold_system(
    client_datasets, hidden_dim=64, num_layers=2, dropout=0.2,
    batch_size=32, learning_rate=0.001, personalization_lr=0.0001,
    rounds=10, local_epochs=5, personalization_epochs=3, 
    noise_scale=0.1, clip_norm=1.0, encoder_noise_scale=0.05,
    ...
    """
    Train a privacy-preserving SCAFFOLD federated learning system.
    ...
    # Create privacy-preserving SCAFFOLD client models
    print("Creating privacy-preserving SCAFFOLD client models...")
    client_models = []
    
    for client_id in client_datasets.keys():
        model = PrivacyPreservingScaffoldModel(
            hf_input_dim=hf_input_dim,
            lf_input_dim=lf_input_dim,
            lf_output_dim=lf_output_dim,
            hidden_dim=hidden_dim,
    ...
    # Create privacy-preserving SCAFFOLD system
    print("Initializing privacy-preserving SCAFFOLD system...")
    system = PrivacyPreservingScaffoldSystem(
        client_models=client_models,
        client_optimizers=client_optimizers,
        noise_scale=noise_scale,
        clip_norm=clip_norm,
    ...
    # Training loop
    print(f"Starting private SCAFFOLD training for {rounds} rounds with {local_epochs} local epochs each...")
    history = {
        'train_metrics': [],
        'personalization_metrics': [],
        'val_metrics': [],
    ...
    for round_idx in range(rounds):
        ...
        # Step 1: Privacy-preserving SCAFFOLD training
        train_metrics = system.train_round(
            client_data_loaders=train_loaders,
            local_epochs=local_epochs,
            client_weights=client_weights
        ...
        if avg_personalized_val_loss < history['best_personalized_val_loss']:
            ...
            best_model_state = {
                'client_models': [model.state_dict() for model in system.client_models],
                'personalized_models': [model.state_dict() for model in system.personalized_models],
                'server_control_variate': system.server_control_variate.get_dict(),
                'client_control_variates': [cv.get_dict() for cv in system.client_control_variates]
            }
    ...
    if best_model_state:
        ...
        for i, model in enumerate(system.personalized_models):
            model.load_state_dict(best_model_state['personalized_models'][i])
        system.server_control_variate.update_from_dict(best_model_state['server_control_variate'])
        for i, cv in enumerate(system.client_control_variates):
            cv.update_from_dict(best_model_state['client_control_variates'][i])
